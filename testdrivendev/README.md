...
- Bank.reduce() -> Sum.reduce() Bank 객체와 환전하고싶은 단위 인자로 받는다. (Bank를 거치는 이유는 Bank의 객체를 받아서 붙어있는 함수를 사용하기 위해서)
...
### 16장, 드디어, 추상화
- Sum 클래스에서 Sum 객체의 augend, tenFrancs 각각의 해당하는 숫자를 곱해주는 함수 생성
...
#### 또 다른 테스트
- 주제와 무관한 아이디어가 떠오르면 이에 대한 테스트를 할일 목록ㄷ에 적어두고 다시 본론으로 들어온다.
